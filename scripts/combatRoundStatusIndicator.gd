...
func toggle_turn_panels(atk: bool, pitch: bool, def: bool, pitch_def : bool):
	...
	$PitchPanel2.set_texture(LIT_PANEL_TEXTURE if pitch_def else UNLIT_PANEL_TEXTURE)
...
func status(state):
	...
	match state:
		...
		Gs.GameState.PL_WAITING_FOR_CARD, Gs.GameState.PL_RESOLVING_ATTACK_CARD, Gs.GameState.PL_RESOLVING_SPELL_CARD, Gs.GameState.PL_NOT_ENOUGH_MANA_FOR_CARD:
			cue_attack_hint()
			toggle_turn_panels(true, false, false, false)
			pitch_atk = true
		...
		Gs.GameState.PL_WAITING_FOR_PITCHED_CARDS, Gs.GameState.PL_RESOLVING_PITCHED_CARDS, Gs.GameState.PL_PITCHING_PHASE_FINISHED:
			...
			if pitch_atk == true:
				toggle_turn_panels(false, true, false, false)
				
			else:
				toggle_turn_panels(false, false, false, true)
		...
		Gs.GameState.EM_ATTACK, Gs.GameState.PL_RESOLVING_BLOCKING_PHASE, Gs.GameState.PL_RESOLVING_BLOCKING_CARD, Gs.GameState.PL_BLOCKING_PHASE_FINISHED:
			cue_defend_hint()
			toggle_turn_panels(false, false, true, false)
			pitch_atk = false
		# Player Pitching for Defense Status
		#Gs.GameState.PL_WAITING_FOR_PITCHED_CARDS, Gs.GameState.PL_RESOLVING_PITCHED_CARDS, Gs.GameState.PL_PITCHING_PHASE_FINISHED:
			#toggle_turn_panels(false, false, false, true)
		_:
			pass
